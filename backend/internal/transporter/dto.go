...
type CreateTransporterRequest struct {
	...
	VehicleTypes    []VehicleType     `json:"vehicle_types" validate:"required"`
	MaxCapacity     TransportCapacity `json:"max_capacity" validate:"required"`
	Specializations []string          `json:"specializations"`
}

type AddVehicleRequest struct {
	VehicleNumber string      `json:"vehicle_number" validate:"required"`
	VehicleType   VehicleType `json:"vehicle_type" validate:"required,oneof=truck refrigerated_truck pickup van tractor"`
	Make          string      `json:"make" validate:"required"`
	Model         string      `json:"model" validate:"required"`
	Year          int         `json:"year" validate:"min=1900"`
	Color         string      `json:"color"`

	Capacity TransportCapacity `json:"capacity" validate:"required"`
...
type TransporterResponse struct {
	...
	MaxCapacity  TransportCapacity `json:"max_capacity"`
...
type VehicleResponse struct {
	...
	Capacity TransportCapacity `json:"capacity"`
